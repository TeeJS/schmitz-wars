...
| **M3** ✓ | **Gates A and B passed 2026-09-03** (the GHCR image `ghcr.io/teejs/wars-relay` built by GitHub Actions, deployed on Unraid behind NPM Plus at wars.schmitzplex.com; two headless clients through the live container 200/200; the wss leg by curl and the browser). **Relay service** on Unraid behind NPM Plus (TLS, `wars-relay.schmitzplex.com`), game list, create/join, log store. Web build talks to it. | Two browsers on two machines play 50 days; hashes identical; both logs identical. | 2–3 days (+ TeeJ for the NPM Plus host and DNS) |
