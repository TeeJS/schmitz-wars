...
| Modem answer / serial dialogs (Figs 5.4, 5.5) | p158–p159 | *n/a* - no modem or serial in a browser. |
| **Locate Session** / TCP-IP dialog (Fig 5.6): "Enter the computer name or IP address of the session host, or leave blank to search" | p159 | Becomes the relay's game list: leave blank and the Join screen lists every game waiting on the relay; or type a game code to join a private one. |
...
| **Multiplayer Options screen** (Fig 5.9), host only: **choose your side** (red Alliance / green Empire symbols), **galaxy size** standard/large/huge, **Standard Game / HQ Only Victory**, **Load Game** (only if a save with this opponent exists), **chat** ("click to the right of Chat>, type, Enter"), chat/settings view, **checkmark** Start Game (host), previous, cancel | p161–p162 | Same screen. Note: no difficulty choice - head-to-head uses the **Multiplayer** column of every rule table, which the port already carries (`Enums.Difficulty.Multiplayer`, `side_lottery.json` "mp"). The guest sees the host's choices and the chat, and waits. |
| **Messages (Chat)**: "processed through SD-7 or R2-D2's messaging system"; **Chat Messages** tab in the **Display Message Index** (Fig 5.10); "the button on the bottom right-hand side of the window to send a message"; **Compose Chat Message** window (Fig 5.11): type, **Send**, **Cancel**, close, return to index | p162–p163 | `MessageCategory.Chat` and the Chat tab already exist in MessageWindow. Add the compose button and the Compose Chat Message window; a chat line becomes a `GameMessage` in the Chat category on both clients. |
...
| **Pausing**: Game Options screen → "Your opponent will receive a **Waiting for Opponent** message until you return"; or Pause on the Game Speed menu, "click on the checkbox to resume play" | p163 | Pause is a command; the other client shows "Waiting for Opponent" (modal, like the single-player pause box) until the pauser resumes. |
| **Saving**: "only the host player can save the game. Star Wars Rebellion will create a saved game on both computers in the same saved game slots"; Load only from the Multiplayer Options screen, "only available if you have saved a game from a previous session with your current opponent" | p163–p164 | The order log IS the save (§3). Host-only Save writes the log to the relay's store and to both browsers' storage; Load lists saves whose two player names match. |
