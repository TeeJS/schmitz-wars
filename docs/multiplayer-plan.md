...
| **M5** ◐ | **Save / Load / reconnect** (engine side done 2026-09-03: rejoin gate 171/171, `tools/lockstep-local.ps1 -Relay -Rejoin 30`; the screens wait on C3PO): host-only Save = log to relay + both browsers' storage; Load from Multiplayer Options when a save with the same two names exists; drop and rejoin mid-game. | Rejoin after a kill: hash equal to the surviving client's. Load: game resumes at the saved day with identical hash. | 2 days |
