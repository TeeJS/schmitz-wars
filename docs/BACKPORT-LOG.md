...
Hashes:    unchanged. Reports land on the box under /mnt/user/appdata/wars-relay/data/feedback/. Gates: relay test (accepted, files written, junk and empty refused); `tools/feedback-local.ps1` (a headless single-player game submits a note through a local relay; the report's player/game/day/seed/log lines are checked).
